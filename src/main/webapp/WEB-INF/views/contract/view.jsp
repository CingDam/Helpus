...
<body>
	...
	<div id="wrap">
		<div id="section">
			<div id="content1">
				<div>계약서</div>
				<div>${item.contract.userName}(이하 “동”이라 한다)와
					${item.contract.coName} (이하 “행”이라 한다)는 다음과 같이 계약을 체결한다.</div>
			</div>
			...
			<div id="content2">
				<div id="checkbox" class="mb-2 mt-2">
					<c:forEach var="items" items="${item.coKey }">
						<div class="form-check">
							<input class="form-check-input" type="radio" name="keyCode"
								value="${items.keyCode}"
								${item.contract.keyCode == items.keyCode ? 'checked' : ''}
								onclick="return(false);"> <label
								class="form-check-label stop-dragging">${items.keyName}</label>
						</div>
					</c:forEach>
				</div>
				<div class="mb-2 row">
					<fmt:formatDate value="${item.contract.contractSdate}"
						pattern="yyyy-MM-dd" var="sdate" />
					<fmt:formatDate value="${item.contract.contractEdate}"
				...
				<div class="mb-2 row">
					<fmt:formatDate value="${item.contract.contractSdate}"
						pattern="HH:mm" var="stime" />
					<fmt:formatDate value="${item.contract.contractEdate}"
						pattern="HH:mm" var="etime" />
					<label class="col-md-2 col-form-label">시간 </label>
					<div class="col-md-10 col-form-label">
						<c:out value="${stime}" />
						~
						<c:out value="${etime}"></c:out>
					</div>
				</div>
				<div class="mb-2 row">
					<label class="col-md-2 col-form-label">금액</label>
					<div class="col-md-6 col-form-label">
						${item.contract.contractPay}</div>
				</div>
			...
			<div id="content3">
				<div>
					...
					<div class="row">
						...
						<div class="col-md-5">
							<input id="sign" class="form-control" type="text"
								placeholder="${item.contract.userName }" />
						</div>
					</div>
				</div>
			...
			<div id="button">
				...
				<button style="margin: 0 0 0 20px;" class="btn btn-primary btn-lg"
					id="addContract" onclick="location.href='../../'">돌아가기</button>
			</div>
		</div>
	</div>
</body>
